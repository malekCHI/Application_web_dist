...
public class ChapitreController {
    ...
    @GetMapping("/getChapitresbyCoursid/{coursId}")
    public List<Chapitre> getCoursChapitres(@PathVariable("coursId") Long id) {
        return chapitreimpl.findChapitresByCours_Id(id);
    }

    @PostMapping("/markChapitreAsCompleted/{id}")
    public ResponseEntity<String> markChapitreAsCompleted(@PathVariable("id") Long id) {
        chapitreimpl.markChapitreAsCompleted(id);
        return ResponseEntity.ok("Chapter marked as completed successfully");
    }
}
